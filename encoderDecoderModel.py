...
class EncoderRNNPre(nn.Module):
    ...
    def forward(self, x):
        ...
        # Combine forward and backward states for encoder output
        encoder_states = encoder_states.view(encoder_states.size(0), encoder_states.size(1), 2, -1)
        encoder_states = torch.cat([encoder_states[:, :, 0, :], encoder_states[:, :, 1, :]], dim=2)
        
        return encoder_states, hidden


class DecoderRNNPre(nn.Module):
    def __init__(self, num_embeddings, embedding_dim, input_size, hidden_size,  num_layers, dropout, vocab_size) -> None:
        super().__init__()
        self.embed = nn.Embedding(num_embeddings=num_embeddings, embedding_dim=embedding_dim)
        self.lstm = nn.GRU(input_size=input_size, hidden_size=hidden_size * 2, num_layers=num_layers, bidirectional=False, dropout=dropout)
        self.fc = nn.Linear(in_features=hidden_size * 2, out_features=vocab_size)
